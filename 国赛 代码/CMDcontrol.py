...
def action_request(param, ser):
    ...
    if(runningAction==True and actionComplete==False):
        #print(" ##### false")
        actionComplete = True
...
def action_wait():
    ...
    if actionComplete == False:
        ...
        with eventlet.Timeout(30,False):
            # #print("11111 ",actionComplete)
            while len(action_list) != 0 or actionComplete == False:
                # #print('wait......')
                time.sleep(0.001)
    # #print("22222 ",actionComplete)
    runningAction = False
...
def action_append(act_name):
    global acted_name , action_list , action_count, actionComplete
    action_count += 1 
    # #print(f'执行：{act_name} req_counter = {req_counter}')
